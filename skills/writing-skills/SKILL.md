...
**Official guidance:** For the agent's official skill authoring best practices, see references/anthropic-best-practices.md. This document provides additional patterns and guidelines that complement the TDD-focused approach in this skill.
...
A skill must reside in its own subdirectory, and the name of the directory must match the `name` field in the `SKILL.md` frontmatter.

```text
skill-name/
├── SKILL.md      # Required: metadata and instructions
├── scripts/      # Optional: executable code (helpers, binaries, scripts)
├── references/   # Optional: documentation (heavy reference docs, API guides)
├── assets/       # Optional: static assets (templates, boilerplate, config)
└── ...           # Any additional supporting files
```

**Key Components & Roles:**

- **`SKILL.md`**: Main instructions loaded when the skill is triggered.
- **`scripts/`**: Executable helper scripts, CLI tools, or utilities the agent can run.
- **`references/`**: Heavy documentation, API guides, or design specs read on-demand.
- **`assets/`**: Static templates, boilerplate configs, or non-executable resources.
...
**Critical for discovery:** The future AI agent needs to FIND your skill
