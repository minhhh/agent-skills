...
- **Fabrication / content invention**: This skill only extracts, reformats, and summarizes content that exists in the provided source material (documentation, book, URL, file). You MUST NOT invent, generate, or "deepen" with technical details — protocol internals, performance numbers, failure modes, architecture descriptions, or any other content — that are not present in the source. If the source is brief on a topic, preserve that brevity; do not "fill in" from external knowledge or general expertise. The result must be defensible from the source alone.
