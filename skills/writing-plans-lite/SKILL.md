...
1. **Bootstrap & Plan**: Create `.tasks/prd-mvp.md` (or `prd-[feature].md` for
   ...
   - **Assets Folder**: For complex features or features that involve reference templates, config files, or mockups, you MUST create a dedicated folder at `.tasks/prd-[feature]/` (e.g. `.tasks/prd-kb-to-markdown/`) to store these resources and link to them within the PRD. All links to these assets and other local repository files MUST be relative paths (e.g., `[label](prd-[feature]/file.md)`), never absolute `file:///` URLs.
